...
async def clear_catalog() -> int:
    ...
    deleted_bikes_amount = await db_pool.scard(ACTUAL_CATALOG_KEY)  # type: ignore
...
async def insert_actual_catalog(actual_catalog: List[Bike]) -> int:
    ...
    for bike_item in actual_catalog:
        ...
        await db_pool.sadd(ACTUAL_CATALOG_KEY, bike_item.id)  # type: ignore
        await db_pool.hset(BIKE_KEY.format(bike_item.id), mapping=asdict(bike_item))  # type: ignore

    await db_pool.set(CATALOG_UPDATE_DATE_KEY, str(datetime.datetime.utcnow()))

    return await db_pool.scard(ACTUAL_CATALOG_KEY)  # type: ignore


async def get_catalog() -> List[Bike]:
    """Get actual catalog from the database. Return list of bikes in elements."""
    list_bike_id = await db_pool.smembers(ACTUAL_CATALOG_KEY)  # type: ignore

    output: List[Bike] = []
    for bike_id in list_bike_id:
        bike_item: Bike = Bike(
            **await db_pool.hgetall(BIKE_KEY.format(bike_id)),  # type: ignore
        )
